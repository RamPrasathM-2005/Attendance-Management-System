  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-800">{semester.department} - Semester {semester.semester}</h2>
          <p className="text-gray-600">Batch: {semester.batch}</p>
        </div>
      </div>
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {(semesterCourses[semester.id] || []).map((c) => (
            <CourseCard key={c.id} course={c} />
          ))}
        </div>
        {(!semesterCourses[semester.id] || semesterCourses[semester.id].length === 0) && (
